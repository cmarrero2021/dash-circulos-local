<template>
  <div ref="tableContainer" class="pivot-table-container">
    <div v-if="tableData.bodyRows?.length" class="pivot-info-bar">
      ...
      <span v-if="store.totalRows" class="total-label">de {{ store.totalRows }} registros</span>
    </div>
      ...
      <table v-if="tableData.headers?.length" class="pivot-table">
        ...
        <thead>
          ...
          <tr v-if="tableData.hasPivotColumns" class="pivot-header-top">
            <th
v-for="h in tableData.headers.filter(h => h.isRowHeader)" :key="'rh-'+h.key"
...
v-for="h in tableData.headers" :key="'sh-'+h.key" class="pivot-th"
              :class="{ 'sort-active': sortColumn === h.key }" @click="sortBy(h.key)">
              {{ h.label }}
...
v-for="h in tableData.headers" :key="'d-'+h.key"
            ...
            <template v-else>
              <td
v-for="h in tableData.headers.filter(h => h.isRowHeader)" :key="'dr-'+h.key"
                class="pivot-td pivot-td-row">{{ formatValue(row[h.key]) }}</td>
              <td
v-for="h in tableData.headers.filter(h => h.isValue)" :key="'dv-'+h.key"
...
v-if="!tableData.hasPivotColumns"
            ...
            <template v-if="!tableData.hasPivotColumns">
              <td
v-for="h in tableData.headers.filter((h,i) => i >= (store.pivotRows.length || 1))"
            ...
            <template v-else>
              <td
v-for="h in tableData.headers.filter(h => h.isValue)" :key="'gtv-'+h.key"
