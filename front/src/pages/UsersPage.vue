  ...
  <q-page padding>
    ...
    <q-dialog v-model="editDialog.show">
      <q-card style="width: 600px; max-width: 90vw;">
        <q-card-section>
          <div class="text-h6">Editar Usuario: {{ editDialog.user.nombre }}</div>
        </q-card-section>
...
async function openEditDialog(user) {
  editDialog.value.user = { ...user }; // Copia para no modificar el original
...
async function saveUserChanges() {
  ...
  try {
    ...
    // 2. Guardar los estados permitidos si es rol Estadal
    ...
    await fetchData(); // Recargar la lista de usuarios
